...
This directory is **shared by Codex and Claude Code**. Its layout is deliberate — keep it clean.
...
- **`conversations/`** holds everything else: ad-hoc reports, data dumps, generated scripts, images, PDFs, scratch analysis — anything produced during a chat that isn't part of an existing repo.
  - `conversations/codex/` — your output
  - `conversations/claude/` — Claude Code's output
...
```
conversations/codex/<YYYY-MM-DD>_<short_topic_slug>/
```

- Use **today's actual date** (run `date +%F` if unsure). Example: `conversations/codex/2026-06-05_projects_directory_organization/`.
...
A **one-off** gets a dated folder (above). An **automation** — the same task run repeatedly (a weekly report, a daily data pull) — uses a different shape: the task is the folder, and dates live on the output *inside* it.

The moment you run something a second time, promote it to an automation:

```
conversations/codex/automations/<task-name>/
